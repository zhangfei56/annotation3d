...
class Rect2D extends BaseShape {
  ...
  private createAssistPoints() {
    Object.keys(AssistPointDefined).forEach((key) => {
      ...
      const assistPoint = new AssistPoint(
        new THREE.Vector3(
          this.keyPoint[positionDefined.x],
          this.keyPoint[positionDefined.y],
          0,
        ),
        this,
        this.assistPointMove(positionDefined).bind(this),
      );
  ...
  public assistPointMove(positionDefined: { x: 'minX' | 'maxX'; y: 'minY' | 'maxY' }) {
