...
export class InputEmitter extends EventEmitter {
  ...
  onMouseMove(event: MouseEvent) {
    ...
    if (imageIntersections.length > 0) {
      if ((this.clickedObjects?.length ?? 0) > 0) {
        this.clickedObjects!.forEach((item) =>
          item.userData.selfClass.mouseMoveHandler(this.currentPosition),
        );
        return;
      }

      const listerIntersections = this.raycaster.intersectObjects(
      ...
      enterObjects.forEach((item) =>
        item.userData.selfClass.mouseEnterHandler(this.currentPosition),
      );

      leaveObjects = this.movedObjects.filter((item) => !movingObjects.includes(item));
      leaveObjects.forEach((item) =>
        item.userData.selfClass.mouseLeaveHandler(this.currentPosition),
      );
  ...
  private onMouseDown(event: MouseEvent) {
    ...
    if (imageIntersections.length > 0) {
      ...
      if (listerIntersections.length > 0) {
        ...
        this.clickedObjects.forEach((item) =>
          item.userData.selfClass.mouseDownHandler(this.currentPosition),
        );
        return;
      }
  ...
  onMouseUp(event: MouseEvent) {
    ...
    if ((this.clickedObjects?.length ?? 0) > 0) {
      this.clickedObjects!.forEach((item) =>
        item.userData.selfClass.mouseUpHandler(this.currentPosition),
      );
